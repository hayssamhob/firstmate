...
| `fm-spawn.sh`            | Spawn one task, several `id=repo` pairs, or a persistent secondmate with `--secondmate`; accepts concrete `--harness`, `--model`, `--effort`, and `--backend` axes; ship/scout spawns require an explicit resolved harness when dispatch profiles are active and an isolated treehouse worktree, install per-harness turn-end signaling (plus, for non-secondmate devin spawns, a git-excluded `PreToolUse` merge-deny guard that blocks merge and default-branch-push commands), inject the opt-in Claude Foreman crew identity into GitHub-origin project spawns (pane-environment only; `foreman=` and a private 0700 `foremantmp=` dir recorded in meta), and secondmate spawns resolve the secondmate harness plus optional `config/secondmate-harness` model/effort tokens, locally sync the home, and propagate declared inheritable config before launch |
...
| `fm-teardown.sh`         | Return a clean, landed ship worktree or retire/release a secondmate home; requires scout reports, checks child work, removes firstmate-owned hook artifacts and the per-task Claude Foreman `foremantmp=` dir, and prints the backlog-backend reminder |
