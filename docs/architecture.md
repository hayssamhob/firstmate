...
That keeps spawn launch compatible across claude, codex, grok, pi, opencode, devin, and antigravity while preserving the requested profile for later audit.

## Optional crew GitHub identity
...
As defense in depth, right after the pane sources that env and before the agent launches, `fm_foreman_verify_identity` runs `gh auth status` in the same pane shell and confirms the active gh account is the bot, recording `foreman_verify=ok|mismatch:<account>|unknown` in task meta and warning loudly on a mismatch; it never blocks the spawn (`FM_FOREMAN_VERIFY=0` disables it, `FM_FOREMAN_VERIFY_TIMEOUT` overrides the wait), turning a silent injection failure - the 2026-07-12 self-merge-as-captain incident where the injected identity never reached the merging shell - into a firstmate-detectable signal ([docs/devin-merge-deny.md](devin-merge-deny.md)).
