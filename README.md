...
An opt-in crew GitHub App identity (local `config/claude-foreman.env` plus `config/claude-foreman.pem`) makes crewmate and scout spawns into GitHub-origin projects author commits and PRs as the app bot, via per-task repo-scoped installation tokens injected into the pane environment only; without the files spawns are byte-identical, and secondmate spawns are exempt.
As defense in depth, each spawn then verifies that identity is actually active in the pane and records `foreman_verify=` in task meta, warning loudly if a crewmate is authing as the wrong account rather than the bot.
A companion captain-consented login-PATH `gh` wrapper (`bin/fm-foreman-gh-shim.sh`, surfaced by bootstrap as `FOREMAN_DAEMON_SHIM:` lines) makes PRs opened by the no-mistakes daemon outside the pane author as the bot too, interposing only `gh pr create` from a no-mistakes ancestor and leaving all other `gh` use untouched.
