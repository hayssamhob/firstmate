...
# foremantmp is recorded by fm-spawn when the Claude Foreman crew identity set
# up a private per-task dir (token cache + gh shim); absent otherwise.
FOREMAN_TMP=$(grep '^foremantmp=' "$META" | cut -d= -f2- || true)
...
[ -z "$FOREMAN_TMP" ] || rm -rf "$FOREMAN_TMP"
