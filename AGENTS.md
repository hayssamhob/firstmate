...
As defense in depth (the 2026-07-12 incident where a devin crewmate self-merged a PR as the captain because the injected identity never reached the merging shell), right after the pane sources that env and before the agent launches, `fm-spawn.sh` runs `gh auth status` in the same pane shell and confirms the active gh account is the bot, recording `foreman_verify=ok|mismatch:<account>|unknown` in meta and warning loudly on a mismatch; it never blocks the spawn (`FM_FOREMAN_VERIFY=0` disables it), and a `foreman_verify=mismatch:` line is an actionable signal that the crewmate is authing as the wrong account (docs/devin-merge-deny.md).
